...
export class LoadingViewComp extends CCVMParentComp {
    ...
    reset(): void {
        // 获取用户信息的多语言提示文本
        this.data.prompt = oops.language.getLangByID("text_loading_res");

        // 进入自定义游戏内容界面
        oops.gui.open(UIID.Start);
    ...
    private onProgressCallback(finished: number, total: number, item: any) {
        ...
        if (progress > this.progress) {
            ...
            let x = -260 + 520 * this.progress;
            this.node.getChildByName("pro_progress").getChildByName("img_progress").setPosition(x, 0, 0);
        }
    }
